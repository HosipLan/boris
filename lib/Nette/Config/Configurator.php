...
class NConfigurator extends NObject
{
	...
	public function enableDebugger($logDirectory = NULL, $email = NULL)
	{
		...
		NDebugger::enable(!$this->parameters['debugMode'], $logDirectory, $email);
	}
		...
		foreach ($this->files as $tmp) {
			...
			try {
				if ($section === NULL) { // back compatibility
					$config = NConfigHelpers::merge($loader->load($file, $this->parameters['environment']), $config);
					continue;
				}
			} catch (NAssertionException $e) {}
	...
	protected function createCompiler()
	{
		...
		$compiler->addExtension('php', new NPhpExtension)
			...
			->addExtension('extensions', new NExtensionsExtension);
		return $compiler;
	}
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use setDebugMode(!$value) instead.', E_USER_WARNING);
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use !isDebugMode() instead.', E_USER_WARNING);
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use !detectDebugMode() instead.', E_USER_WARNING);
