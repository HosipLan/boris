...
class NFinder extends NObject implements IteratorAggregate
{
	...
	private function buildIterator($path)
	{
		...
		if ($this->exclude) {
			...
			$iterator = new NNRecursiveCallbackFilterIterator($iterator, create_function('$foo, $foo, $file', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('filters'=>$filters)).'-1], EXTR_REFS);
		...
		if ($this->groups) {
			$groups = $this->groups;
			$iterator = new NNCallbackFilterIterator($iterator, create_function('$foo, $foo, $file', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('groups'=>$groups)).'-1], EXTR_REFS);
