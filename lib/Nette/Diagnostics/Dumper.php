...
class NDebugDumper
{
	...
	{
		return htmlspecialchars_decode(strip_tags(preg_replace_callback('#<span class="nette-dump-(\w+)">|</span>#', create_function('$m', '
	...
	private static function dumpVar(&$var, array $options, $level = 0)
	{
		if (method_exists(__CLASS__, $m = 'dump' . gettype($var))) {
			return call_user_func_array(array(__CLASS__, $m), array(&$var, $options, $level));
		} else {
	...
	{
		foreach (PHP_VERSION_ID < 50205 ? debug_backtrace() :debug_backtrace(FALSE) as $item) {
			if (isset($item['file']) && strpos($item['file'], dirname(__FILE__)) === 0) {
				continue;
