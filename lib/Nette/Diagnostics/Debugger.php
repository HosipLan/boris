...
final class NDebugger
{
	...
	public static function _init()
	{
		...
		self::$consoleColors = & NDebugDumper::$terminalColors;
	...
	public static function log($message, $priority = self::INFO)
	{
		...
		if ($message instanceof Exception) {
			...
			$message = NDebugDumper::toText($message);
		}
	...
	public static function _shutdownHandler()
	{
		...
		if (isset($types[$error['type']])) {
			$exception = new FatalErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'], NULL);
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use own error handler instead.', E_USER_WARNING);
	...
	public static function dump($var, $return = FALSE)
	{
		if ($return) {
			ob_start();
			NDebugDumper::dump($var, array(
				NDebugDumper::DEPTH => self::$maxDepth,
				NDebugDumper::TRUNCATE => self::$maxLen,
			));
			return ob_get_clean();
			...
			NDebugDumper::dump($var, array(
				NDebugDumper::DEPTH => self::$maxDepth,
				NDebugDumper::TRUNCATE => self::$maxLen,
				NDebugDumper::LOCATION => self::$showLocation,
			));
		}
	...
	public static function barDump($var, $title = NULL)
	{
		if (!self::$productionMode) {
			...
			foreach ((is_array($var) ? $var : array('' => $var)) as $key => $val) {
				$dump[$key] = NDebugDumper::toHtml($val);
			}
