...
if (!empty($_control->snippetMode)) {
	...
	public function macroBlockEnd(NMacroNode $node, NPhpWriter $writer)
	{
		if (isset($node->data->name)) { // block, snippet, define
			if ($node->name === 'snippet' && $node->prefix === NMacroNode::PREFIX_NONE // n:snippet -> n:inner-snippet
	...
	{
		if ($node->name === 'widget') {
			trigger_error('Macro {widget} is deprecated; use {control} instead.', E_USER_WARNING);
		}
