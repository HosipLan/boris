...
class NLatteCompiler extends NObject
{
	...
	private function processText(NLatteToken $token)
	{
	...
	private function processMacroTag(NLatteToken $token)
	{
	...
	private function processHtmlAttribute(NLatteToken $token)
	{
		if (NStrings::startsWith($token->name, NParser::N_PREFIX)) {
			...
			if (isset($this->htmlNode->macroAttrs[$name])) {
				...
				throw new NCompileException("Macro-attributes must not appear inside macro; found $token->name inside {{$this->macroNode->name}}.", 0, $token->line);
			}
	...
	public function expandMacro($name, $args, $modifiers = NULL, $nPrefix = NULL)
	{
		...
		foreach (array_reverse($this->macros[$name]) as $macro) {
			$node = new NMacroNode($macro, $name, $args, $modifiers, $this->macroNode, $this->htmlNode, $nPrefix);
