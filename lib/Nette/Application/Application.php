...
class NApplication extends NObject
{
	...
	public function run()
	{
		...
		do {
			try {
				...
				if ($repeatedError) {
					$e = new NApplicationException("An error occurred while executing error-presenter '$this->errorPresenter'.", 0, $e);
				}
				...
				$repeatedError = TRUE;
				$this->errorPresenter = ($tmp=$this->errorPresenter) ? $tmp : 'Nette:Error';
				...
				if ($this->presenter instanceof NPresenter) {
					...
					$request = new NPresenterRequest($this->errorPresenter, NPresenterRequest::FORWARD, array('exception' => $e));
				}
				// continue
			}
		} while (1);
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use $presenter->storeRequest() instead.', E_USER_WARNING);
	...
	{
		trigger_error(__METHOD__ . '() is deprecated; use $presenter->restoreRequest() instead.', E_USER_WARNING);
