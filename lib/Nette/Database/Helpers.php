...
class NDatabaseHelpers
{
	...
	public static function dumpSql($sql, array $params = NULL)
	{
		...
		// parameters
		$i = 0;
		$sql = preg_replace_callback('#\?#', create_function('', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('params'=>$params, 'i'=>& $i)).'-1], EXTR_REFS);
			if (!isset($params[$i])) {
				return \'?\';
			}
			$param = $params[$i++];
			if (is_string($param) && (preg_match(\'#[^\\x09\\x0A\\x0D\\x20-\\x7E\\xA0-\\x{10FFFF}]#u\', $param) || preg_last_error())) {
				...
				return \'<span title="Length \' . NStrings::length($param) . \' characters">\\\'\' . htmlspecialchars(NStrings::truncate($param, NDatabaseHelpers::$maxLength)) . "\'</span>";
	...
	{
		$panel = new NDatabasePanel($connection);
